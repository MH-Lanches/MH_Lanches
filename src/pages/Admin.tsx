...
export default function Admin() {
  const { dbSource, setDbSource } = useDatabase();
  ...
  useEffect(() => {
    if (dbSource === 'firebase') {
      ...
      // Local JSON Fallback
      fetch('/config.json').then(r => r.json()).then(setSiteConfig);
      fetch('/produtos.json').then(r => r.json()).then(data => {
        setProducts(data);
        const cats = Array.from(new Set(data.map((p: any) => p.categoria)));
        setCategories(cats as string[]);
      });
      setOrders([]); // Local fallback doesn't have orders for now
      setCoupons([]);
    }
  }, [dbSource]);
