...
export default function PDV() {
  ...
  useEffect(() => {
    ...
    if (dbSource === 'firebase') {
      ...
      // Local JSON Fallback
      fetch('/pedidos.json').then(r => r.json()).then(data => {
        const sortedList = data.sort((a: any, b: any) => new Date(b.data).getTime() - new Date(a.data).getTime());
        setOrders(sortedList);
      }).catch(() => setOrders([]));
    }

    return () => clearInterval(timer);
  }, [prevOrdersCount, dbSource]);
